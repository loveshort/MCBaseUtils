...
- (void)setupSubviews{
    [super mc_setupSubviews];
    
    contentMaxWidth = self.emptyViewIsCompleteCoverSuperView ? self.mc_width : self.mc_width - 30.f;
    ...
    //自定义view
    if (self.customView) {
        contentWidth = self.customView.mc_width;
        contentHeight = self.customView.mc_maxY;
    }
...
- (void)setSubViewFrame{
    ...
    //emptyView初始宽高
    CGFloat originEmptyWidth = self.mc_width;
    CGFloat originEmptyHeight = self.mc_height;
    ...
    if (!self.emptyViewIsCompleteCoverSuperView) {
        self.mc_size = CGSizeMake(contentWidth, contentHeight);
    }
    self.center = CGPointMake(emptyViewCenterX, emptyViewCenterY);
    
    //设置contentView
    self.contentView.mc_size = CGSizeMake(contentWidth, contentHeight);
    if (self.emptyViewIsCompleteCoverSuperView) {
    ...
    //子控件的centerX设置
    CGFloat centerX = self.contentView.mc_width * 0.5f;
    if (self.customView) {
        self.customView.mc_centerX      = centerX;
        
    }else{
        _promptImageView.mc_centerX = centerX;
        _titleLabel.mc_centerX       = centerX;
        _detailLabel.mc_centerX = centerX;
        _actionButton.mc_centerX    = centerX;
    }
    
    if (self.contentViewOffset) { //有无设置偏移
        self.mc_centerY += self.contentViewOffset;
        
    } else if (self.contentViewY < 1000) { //有无设置Y坐标值
        self.mc_y = self.contentViewY;
    ...
    if (self.ignoreContentInset && [self.superview isKindOfClass:[UIScrollView class]]) {
        ...
        self.mc_centerY -= scrollView.contentInset.top;
        self.mc_centerX -= scrollView.contentInset.left;
        self.mc_centerY += scrollView.contentInset.bottom;
        self.mc_centerX += scrollView.contentInset.right;
    }
}
...
- (void)setupPromptImageView:(UIImage *)img{
    ...
    contentWidth = self.promptImageView.mc_size.width;
    contentHeight = self.promptImageView.mc_maxY;
}
...
- (void)setupTitleLabel:(NSString *)titleStr{
    ...
    contentHeight = self.titleLabel.mc_maxY;
}
...
- (void)setupDetailLabel:(NSString *)detailStr{
    ...
    contentWidth = width > contentWidth ? width : contentWidth;
    contentHeight = self.detailLabel.mc_maxY;
   
}

- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(MCActionTapBlock)btnClickBlock{
    ...
    contentHeight = self.actionButton.mc_maxY;
}
