...
@interface MCCustomNavigationBar ()
...
@implementation MCCustomNavigationBar


+ (instancetype)CustomNavigationBar {
    MCCustomNavigationBar *navigationBar = [[self alloc] initWithFrame:CGRectMake(0, 0, [MCScreenHeightInfo mc_screenWidth], [MCCustomNavigationBar navBarBottom])];
...
-(void)updateFrame {
    NSInteger top = ([MCScreenHeightInfo mc_isIphoneX]) ? 44 : 20;
    NSInteger margin = 0;
...
-(void)clickBack {
    if (self.onClickLeftButton) {
        ...
        UIViewController *currentVC = [UIViewController mc_currentViewController];
        [currentVC mc_toLastViewController];
    }
}
-(void)clickRight {
...
- (void)mc_setLeftButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mc_setLeftButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)mc_setLeftButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self mc_setLeftButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)mc_setLeftButtonWithImage:(UIImage *)image {
    [self mc_setLeftButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)mc_setLeftButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mc_setLeftButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
- (void)mc_setRightButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mc_setRightButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)mc_setRightButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self mc_setRightButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)mc_setRightButtonWithImage:(UIImage *)image {
    [self mc_setRightButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)mc_setRightButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mc_setRightButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
